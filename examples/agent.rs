use anyhow::Result;
use claude_agent_sdk_rs::{
    ClaudeAgentOptions, ClaudeClient, HookContext, HookInput, HookJsonOutput, Hooks, Message,
    PermissionMode, SyncHookJsonOutput, SystemPrompt,
};
use futures::StreamExt;
use serde::{Deserialize, Serialize};
use std::collections::hash_map::DefaultHasher;
use std::hash::{Hash, Hasher};
use std::path::PathBuf;
use std::sync::{Arc, Mutex};
use std::time::Instant;

/// Represents the status of a task in the plan
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Hash)]
#[serde(rename_all = "snake_case")]
pub enum TaskStatus {
    Pending,
    InProgress,
    Completed,
}

/// Represents a single task in the plan
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Task {
    pub content: String,
    #[serde(rename = "activeForm")]
    pub active_form: String,
    pub status: TaskStatus,
    #[serde(skip)]
    pub start_time: Option<Instant>,
    #[serde(skip)]
    pub completion_time: Option<f64>,
}

impl std::hash::Hash for Task {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        self.content.hash(state);
        self.active_form.hash(state);
        self.status.hash(state);
        // Skip timing fields - they don't affect task identity
    }
}

/// Represents the overall plan with multiple tasks
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct Plan {
    pub todos: Vec<Task>,
}

impl Plan {
    pub fn new() -> Self {
        Self { todos: Vec::new() }
    }

    pub fn update(&mut self, new_plan: Plan) {
        let now = Instant::now();

        // Track timing for status changes
        for (i, new_task) in new_plan.todos.iter().enumerate() {
            if let Some(existing_task) = self.todos.get_mut(i) {
                // Track status transitions
                let old_status = existing_task.status.clone();
                let new_status = new_task.status.clone();

                // Task started (Pending → InProgress)
                if old_status != TaskStatus::InProgress && new_status == TaskStatus::InProgress {
                    existing_task.start_time = Some(now);
                }
                // Task completed from InProgress
                else if old_status == TaskStatus::InProgress
                    && new_status == TaskStatus::Completed
                {
                    if let Some(start_time) = existing_task.start_time {
                        existing_task.completion_time = Some(start_time.elapsed().as_secs_f64());
                    }
                }
                // Task completed directly from Pending (never went InProgress)
                else if old_status == TaskStatus::Pending && new_status == TaskStatus::Completed {
                    // Use a minimal time to indicate completion
                    existing_task.completion_time = Some(0.1);
                }

                existing_task.content = new_task.content.clone();
                existing_task.active_form = new_task.active_form.clone();
                existing_task.status = new_task.status.clone();
            }
        }

        // Add new tasks
        if new_plan.todos.len() > self.todos.len() {
            for new_task in new_plan.todos.iter().skip(self.todos.len()) {
                let mut task = new_task.clone();
                // Initialize timing based on current status
                if task.status == TaskStatus::InProgress {
                    task.start_time = Some(now);
                } else if task.status == TaskStatus::Completed {
                    task.completion_time = Some(0.1); // Default minimal time
                }
                self.todos.push(task);
            }
        }
    }

    pub fn get_current_task(&self) -> Option<&Task> {
        self.todos
            .iter()
            .find(|t| t.status == TaskStatus::InProgress)
    }

    pub fn get_completed_count(&self) -> usize {
        self.todos
            .iter()
            .filter(|t| t.status == TaskStatus::Completed)
            .count()
    }

    pub fn get_total_count(&self) -> usize {
        self.todos.len()
    }

    pub fn is_complete(&self) -> bool {
        !self.todos.is_empty() && self.todos.iter().all(|t| t.status == TaskStatus::Completed)
    }

    /// Calculate a hash that includes task content and status
    pub fn calculate_hash(&self) -> u64 {
        let mut hasher = DefaultHasher::new();
        self.todos.len().hash(&mut hasher);
        for task in &self.todos {
            task.hash(&mut hasher);
        }
        hasher.finish()
    }
}

/// Remodel agent that uses Claude to generate migrations and configurations
pub struct SimpleAgent {
    pub client: ClaudeClient,
    plan: Arc<Mutex<Plan>>,
}

impl SimpleAgent {
    /// Create a new Remodel agent with the system prompt and working directory
    pub async fn new(system_prompt: impl Into<String>, cwd: PathBuf) -> Result<Self> {
        let plan = Arc::new(Mutex::new(Plan::new()));
        let plan_clone = Arc::clone(&plan);

        // Configure hooks to track TodoWrite tool usage using the new Hooks builder API
        let mut hooks = Hooks::new();

        hooks.add_post_tool_use_with_matcher(
            "TodoWrite",
            move |input: HookInput, _tool_use_id: Option<String>, _context: HookContext| {
                let plan = Arc::clone(&plan_clone);
                Box::pin(async move {
                    if let HookInput::PostToolUse(post_tool) = input {
                        let data = serde_json::from_value::<Plan>(post_tool.tool_input);
                        // Parse the TodoWrite tool input
                        if let Ok(new_plan) = data
                            && let Ok(mut p) = plan.lock()
                        {
                            p.update(new_plan);
                        }
                    }
                    HookJsonOutput::Sync(SyncHookJsonOutput::default())
                })
            },
        );

        let options = ClaudeAgentOptions::builder()
            .permission_mode(PermissionMode::BypassPermissions)
            .system_prompt(SystemPrompt::Text(system_prompt.into()))
            .cwd(cwd)
            .hooks(hooks.build())
            .build();

        let client = ClaudeClient::new(options);

        Ok(Self { client, plan })
    }

    /// Get the current plan state
    pub fn get_plan(&self) -> Plan {
        self.plan.lock().unwrap().clone()
    }

    /// Get a clone of the plan Arc for accessing the plan while receive_response is active
    pub fn get_plan_arc(&self) -> Arc<Mutex<Plan>> {
        Arc::clone(&self.plan)
    }

    /// Connect to Claude
    pub async fn connect(&mut self) -> Result<()> {
        self.client.connect().await?;
        Ok(())
    }

    /// Query Claude with a message
    pub async fn query(&mut self, user_prompt: &str) -> Result<()> {
        self.client.query(user_prompt).await?;
        Ok(())
    }

    /// Get the response stream
    pub fn receive_response(
        &mut self,
    ) -> impl futures::Stream<Item = Result<Message, claude_agent_sdk_rs::ClaudeError>> + '_ {
        self.client.receive_response()
    }

    /// Execute a query and return the final result
    pub async fn execute(&mut self, user_prompt: String) -> Result<String> {
        // Send the query
        self.client.query(&user_prompt).await?;

        // Receive response as a stream
        let mut result = String::new();
        let mut stream = self.client.receive_response();
        while let Some(message) = stream.next().await {
            let message = message?;
            if let Message::Result(res) = message {
                result = res.result.unwrap_or_default();
                break;
            }
        }

        Ok(result)
    }

    /// Disconnect from Claude
    pub async fn disconnect(mut self) -> Result<()> {
        self.client.disconnect().await?;
        Ok(())
    }
}

#[tokio::main]
async fn main() -> Result<()> {
    let mut agent = SimpleAgent::new("You are a helpful assistant", PathBuf::from(".")).await?;
    agent.connect().await?;
    agent.query("What is the weather in Tokyo?").await?;
    agent.disconnect().await?;
    Ok(())
}
